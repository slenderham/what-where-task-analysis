...
def generate_perturbation_matrix(all_models, n_orthogonal_directions=5):
    """
    Generate perturbation matrix for each model.
    For each model, generates:
    1. One rank-one perturbation matrix aligned with the outer product of the choice readout and either dv axis
    N. N rank-one perturbation matrices orthogonal to the choice readout and either dv axis, for each block type
    ...
    Returns:
        perturbation_matrices: List of lists, where each inner list contains:
                             [aligned_perturbation_matrix, orthogonal_perturbation_matrix_1, ..., orthogonal_perturbation_matrix_N]
    """
    aligned_perturbation_matrices = []
    orthogonal_perturbation_matrices = []
    
    for model_idx, model in enumerate(all_models):
        # Extract block type readout weights (hidden_size * num_areas, 2)
        action_weights = model.h2o['action'].effective_weight().detach()
        dv_loc_weights = model.h2o['dv_loc'].effective_weight().detach()
        dv_stim_weights = model.h2o['dv_stim'].effective_weight().detach()
        # Get the readout direction (first component of the 2D output)
        action_direction = action_weights[1]-action_weights[0]  # Shape: (hidden_size)
        dv_loc_direction = dv_loc_weights[1]-dv_loc_weights[0]  # Shape: (hidden_size)
        dv_stim_direction = dv_stim_weights[1]-dv_stim_weights[0]  # Shape: (hidden_size)
        # Normalize the action direction
        action_direction = action_direction / torch.linalg.norm(action_direction)
        dv_loc_direction = dv_loc_direction / torch.linalg.norm(dv_loc_direction)
        dv_stim_direction = dv_stim_direction / torch.linalg.norm(dv_stim_direction)
        # Generate orthogonal directions using Gram-Schmidt process
        hidden_dim = action_direction.shape[0]
        ...
        basis = np.stack([action_direction, dv_loc_direction, dv_stim_direction], axis=0).T  # Shape: (hidden_dim, 3)
